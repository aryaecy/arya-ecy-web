...
export default function Navbar(){const {lang,toggle}=useLanguage();const en=lang==='en';return <header className="final-topbar">
  ...
  <nav className="final-main-nav" aria-label={en?'Main menu':'Ana menü'}>
    <a href="/hakkimizda">{en?'About Us':'Hakkımızda'}</a><a href="/hizmetler">{en?'Services':'Hizmetler'}</a><a href="/kutuphane">{en?'Library':'Kütüphane'}</a><a href="/mevzuat">{en?'Legislation':'Mevzuat'}</a><a href="/iletisim">{en?'Contact':'İletişim'}</a><a href="#">ARYA AI</a>
  </nav>
