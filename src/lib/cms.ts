...
export type SiteContent={id?:string;type:ContentType;title:string;excerpt?:string;body?:string;image_url?:string;file_url?:string;published:boolean;published_at?:string;created_at?:string;sort_order?:number};
...
export async function uploadPdf(token:string,file:File):Promise<string>{
 ...
 if(file.type!=='application/pdf'&&!file.name.toLowerCase().endsWith('.pdf')) throw new Error('Yalnızca PDF dosyası yüklenebilir.');
 const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,'-');
 const path=`regulations/${Date.now()}-${safe}`;
 const r=await fetch(`${url}/storage/v1/object/site-documents/${path}`,{method:'POST',headers:{apikey:anon,Authorization:`Bearer ${token}`,'Content-Type':'application/pdf','x-upsert':'false'},body:file});
 if(!r.ok) throw new Error('PDF yüklenemedi. Supabase Storage kurulumu kontrol edilmeli.');
 return `${url}/storage/v1/object/public/site-documents/${path}`;
}
